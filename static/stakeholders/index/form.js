document.addEventListener("DOMContentLoaded", () => {
  ...
  document.body.style.backgroundColor =
    document.querySelector("#bg-color").innerHTML;
  ...
  document.querySelectorAll(".input-form-title").forEach((title) => {
    title.addEventListener("input", function () {
      ...
      document.title = `${this.value} - StakeHolder Form`;
  ...
  document
    .querySelector("#input-form-description")
  ...
  document.querySelectorAll("#send-form-btn").forEach((btn) => {
    ...
    window.onclick = (e) => {
      if (e.target == document.querySelector("#send-form"))
        document.querySelector("#send-form").style.display = "none";
    };
  });
  ...
  document.querySelector("#setting-form").addEventListener("submit", (e) => {
    ...
    fetch("edit_setting", {
      ...
      body: JSON.stringify({
        ...
        authenticated_responder: document.querySelector(
          "#authenticated_responder"
        ).checked,
        confirmation_message: document.querySelector("#comfirmation_message")
          .value,
    ...
    if (!document.querySelector("#collect_email").checked) {
      if (document.querySelector(".collect-email"))
        document
          .querySelector(".collect-email")
          .parentNode.removeChild(document.querySelector(".collect-email"));
    } else {
      if (!document.querySelector(".collect-email")) {
  ...
  const changeType = () => {
    document.querySelectorAll(".input-question-type").forEach((ele) => {
      ele.addEventListener("input", function () {
        ...
        if (
          this.dataset.origin_type === "multiple choice" ||
          this.dataset.origin_type === "checkbox"
        ) {
          ...
          document.querySelectorAll(".question-box").forEach((question) => {
            ...
            if (
              (this.value === "multiple choice" || this.value === "checkbox") &&
              question.dataset.id === this.dataset.id
            ) {
              fetch(`get_choice/${this.dataset.id}`, {
                method: "GET",
              })
    ...
    fetch("add_question", {
      ...
      .then((result) => {
        ...
        ele.innerHTML = `
            ...
            `;
        document.querySelector("#question-container").appendChild(ele);
        editChoice();
        removeOption();
        changeType();
        editQuestion();
